...
const { WebRTCStreamManager } = require('@networkoptix/webrtc-stream-manager');
...
const systemAuthEndopint = (systemId) =>
...
const compatibleOnlineSystem = (system) => system.stateOfHealth === 'online' && parseFloat(system.version) >= 5.1;
...
const startStream = (relayUrl, cameraId, serverId) => {
  ...
  WebRTCStreamManager.connectWithAccessToken(webRtcUrlFactory, systemToken.access_token, videoElement)
...
const systemSelected = async () => {
  document.querySelector('#selectedCamera').style.display =
    'block';
  ...
  const trafficRelayHost = await fetch(`${import.meta.env.PROD ? '/.netlify/functions/proxy?url=' : 'http://localhost:4242/'}${cloudInstance}/api/utils/settings`)
    ...
    .catch(() =>
      prompt(
        'Unable to fetch traffic relay host from the cloud portal. Please enter the relay URL:',
  ...
  const cameraAvailable = (camera) => ['online', 'recording'].includes(camera.status?.toLowerCase());
  ...
  timeline = await fetch(`https://${systemRelay}/rest/v2/devices/${defaultCamera.id}/footage`, { credentials: 'include' }).then((res) => res.json());
